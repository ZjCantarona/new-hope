...
from .forms import MemberForm, MinistryForm, SpeakerForm
...
def thumbnail_members(request):
    ...
    profile = UserProfile.objects.get_or_create(user=request.user)
    context = {"members": members, "members_active_list": "active", "ministries": ministries, "speakers": speakers, "profile": profile}
    return render(request, template, context)
...
def list_members(request):
    ...
    context = {
        "profile": profile,
        "members": members, "speakers": speakers, "ministries": ministries, "total": len(members),
...
def edit_member(request, pk):
    ...
    ministries = Ministry.objects.all()
    profile = UserProfile.objects.get_or_create(user=request.user)
    context = {"member": member, "form": form, "speakers": speakers, "ministries": ministries, "profile": profile}
    return render(request, template, context)
...
def search_members(request):
    ...
    ministries = Ministry.objects.all()
    profile = UserProfile.objects.get_or_create(user=request.user)
    context = {"ministries": ministries, "speakers": speakers, "profile": profile}
    if q != '':
        qs = Member.objects.active().filter(
            Q(name__icontains=q) | Q(speaker__name__icontains=q) | Q(ministry__name__icontains=q)|
...
@login_required
def get_members_by_speakers(request, speaker):
    template = "members/list.html"
    speakers = Speaker.objects.all()
    ministries = Ministry.objects.all()
    members = Member.objects.active().filter(speaker__name__icontains=speaker)
    profile = UserProfile.objects.get_or_create(user=request.user)
    context = {
        "profile": profile,
        "members": members,
...
def filter_members(request):
    ...
    if speaker is not None:
        initial_members = initial_members.filter(speaker__name__icontains=speaker)
        # import pdb; pdb.set_trace()
        context['members'] = initial_members
...
def add_member(request):
    ...
    ministries = Ministry.objects.all()
    profile = UserProfile.objects.get_or_create(user=request.user)
    context = {"form": form, "speakers": speakers, "ministries": ministries, "members_active_add": "active", "profile": profile}
    return render(request, template, context)
...
def update_ministry(request, pk):
    ...
    form = MinistryForm(request.POST or None, instance=ministry)  # Use request.POST or None
    if form.is_valid():
        form.save()
        messages.success(request, 'Ministry updated successfully.')
        return redirect('list_ministries')
    return render(request, 'ministries/edit.html', {'ministry': ministry, 'form': form})

@login_required
def list_speakers(request):
    template = "speakers/list.html"
    speakers = Speaker.objects.all()
    profile = UserProfile.objects.get_or_create(user=request.user)
    context = {"speakers": speakers, "speakers_active_list": "active", "profile": profile}
    return render(request, template, context)
...
def add_speaker(request):
    # TODO: Make this functionality available only to admins
    template = "speakers/add.html"
    form = SpeakerForm()
    profile = UserProfile.objects.get_or_create(user=request.user)
    context = {"form": form, "speakers_active_add": "active", "profile": profile}
    return render(request, template, context)
...
def create_speaker(request):
    ...
    return render(request, 'speakers/add.html', context)

@login_required
def delete_speaker(request, pk):
    # TODO: Make this functionality available only to admins
    speakers = get_object_or_404(Speaker, pk=pk)
    speakers.delete()
...
def update_speaker(request, pk):
    ...
    return render(request, 'speakers/edit.html', {'speaker': speaker, 'form': form})

@login_required
def detail_speaker(request, pk):
    template = "speakers/detail.html"
    speaker = get_object_or_404(Speaker, pk=pk)
    profile = UserProfile.objects.get_or_create(user=request.user)
    context = {"speaker": speaker, "profile": profile}
    return render(request, template, context)

@login_required
def thumbnail_speaker(request):
    template = "speakers/thumbnail.html"
    speakers = Speaker.objects.all()
...
def api_get_speakers(request):
    speakers = Speaker.objects.all()
    data = {"speakers": speakers}
    return JsonResponse(data, content_type="Application/json", safe=False)


def api_create_speaker(request):
    if request.method == "POST":
        form = SpeakerForm(request.POST, request.FILES or None)
        if form.is_valid():
            speaker = form.save(commit=False)
            speaker.save()
            data = {"STATUS": "OK", "Speaker_ID": speaker.pk}
            return JsonResponse(data, content_type="Application/json", safe=False)
        else:
            data = {"STATUS": "INVALID"}
            return JsonResponse(data, content_type="Application/json", safe=False)
...
def api_edit_speaker(request, pk):
    if request.method == "POST":
        speaker = get_object_or_404(Speaker, pk=pk)
...
def api_delete_speaker(request, pk):
    if request.method == "POST":
        speaker = get_object_or_404(Speaker, pk=pk)
        form = SpeakerForm(request.POST or None, instance=speaker)
        if form.is_valid():
